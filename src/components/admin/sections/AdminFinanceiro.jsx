...
import { Plus, Trash2, DollarSign, RefreshCw, Barcode, UploadCloud, Loader2 } from 'lucide-react';
...
const AdminFinanceiro = () => {
  ...
  const handlePDFUpload = async (e) => {
     ...
     try {
       const { text, date } = await processPDF(file);
       
       // Try extract value (very basic regex)
       // Look for "Valor do Documento" or similar in standard Boletos
       const valueMatch = text.match(/Valor\s*(?:do\s*Documento)?\s*R?\$\s*([\d.,]+)/i);
       let extractedValue = '';
       if (valueMatch) {
          extractedValue = valueMatch[1].replace('.', '').replace(',', '.');
       }

       // Try extract barcode (approx 47 digits)
       const barcodeMatch = text.match(/\d{5}\.?\d{5}\s+\d{5}\.?\d{6}\s+\d{5}\.?\d{6}\s+\d\s+\d{14}/); // Standard layout
       const numericBarcode = text.match(/\d{47}/);

       let extractedBarcode = '';
       if (barcodeMatch) extractedBarcode = barcodeMatch[0];
       else if (numericBarcode) extractedBarcode = numericBarcode[0];

       // Format date if possible
       // Default to extracted date from pdfProcessor logic
       ...
       setFormData(prev => ({
          ...prev,
          descricao: 'Boleto Importado (PDF)',
          valor: extractedValue || prev.valor,
          numero_boleto: extractedBarcode || prev.numero_boleto,
          data_vencimento: isoDate || prev.data_vencimento,
          // Since we can't store the file, we add a note
          link_pagamento: '' 
       }));
       
       toast({ title: "Dados extraídos", description: "Verifique os campos preenchidos. O arquivo PDF não foi salvo (armazenamento indisponível)." });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0">
        <h3 className="text-white font-medium">Controle Financeiro (Boletos Manuais)</h3>
        <div className="flex gap-2 w-full sm:w-auto">
            <Button onClick={fetchData} variant="outline" size="sm" className="shrink-0"><RefreshCw className="w-4 h-4" /></Button>
            <Button onClick={() => setIsModalOpen(true)} className="bg-green-600 hover:bg-green-700 w-full sm:w-auto">
            ...
            <tbody>
              {loading ? <tr><td colSpan="6" className="p-6 text-center">Carregando...</td></tr> : boletos.length === 0 ? <tr><td colSpan="6" className="p-6 text-center">Nenhum boleto manual registrado.</td></tr> : boletos.map((boleto) => (
                <tr key={boleto.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                  ...
                  <td className="px-6 py-4 max-w-xs truncate hidden md:table-cell">{boleto.descricao}</td>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
              <label className="text-xs font-bold text-slate-400 uppercase mb-2 block flex items-center gap-2">
                 <UploadCloud className="w-3 h-3" /> Preencher via PDF
              </label>
              <div className="flex items-center gap-2 w-full">
                  <Input 
                      ...
                      className="text-xs bg-slate-900 border-slate-700 file:mr-4 file:py-1 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-green-600 file:text-white hover:file:bg-green-700 w-full"
                  />
                  {isProcessingPDF && <Loader2 className="w-4 h-4 animate-spin text-green-500 shrink-0" />}
              </div>
          </div>
          ...
          <div className="pt-4 flex justify-end gap-2">
            ...
            <Button type="submit" className="bg-green-600 hover:bg-green-700 w-full sm:w-auto">Salvar Boleto</Button>
          </div>
        </form>
      </Modal>
    </div>
